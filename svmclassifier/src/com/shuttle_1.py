...
data_train = data_normal.head(int(75*total_len_normal/100)).reset_index()
data_test = data_normal.tail(int(25*total_len_normal/100))
data_test_final = pd.concat([data_test,data_abnormal])
data_test_final = data_test_final.reset_index()
X_train = data_train.iloc[:,:-1]
print(len(X_train)) 
y_train = data_train.iloc[:,-1]
print(len(y_train))
X_test = data_test_final.iloc[:,:-1]
print(len(X_test))
y_test = data_test_final.iloc[:,-1]
print(len(y_test))
...
#model_shuttle = clf.fit(X_train)
dump(new_model,open('model.shuttle', 'wb')) 
